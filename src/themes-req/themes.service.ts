...
export class ThemesService {
    ...
    async createTheme(title: string, description: string, request: any): Promise<ThemeDocument> {
        ...
        const createdTheme = new this.themeModel({ title, description, createdBy:user.email });
        return await createdTheme.save();
    }

    async getThemes(page: number = 1, limit: number = 10): Promise<ThemeDocument[]> {
